...
The code listings from this document can be downloaded from \url{www.jenny-owen.co.uk}, plus a \textit{small} amount of supplementary material. The official Player and Stage manuals are, however, much more helpful.


% \subsection*{Acknowledgements}
% I would like to thank Susan Stepney, Jon Timmis and Alan Winfield my supervisors for letting me write this instead of what I'm paid to do. I'd also like to thank everyone who gave feedback on the last edition of this manual, including Wenguo Liu, Ritahani Ishmail, Kelvin Lau, Neil Forster, Andrew Turner, 
...
% TODO got this far. 17/03/10


\section{Building a Robot} \label{sec:buildingRobot}
...
\subsection{Sensors and Devices} \label{sec:robotSensors}
...
\subsubsection{camera}\footnote{http://playerstage.sourceforge.net/doc/Stage-3.2.1/group\_\_model\_\_camera.html}
...
\subsubsection{fiducial}\footnote{http://playerstage.sourceforge.net/doc/Stage-3.2.1/group\_\_model\_\_fiducial.html}
...
\subsubsection{ranger}\footnote{http://playerstage.sourceforge.net/doc/Stage-3.2.1/group\_\_model\_\_ranger.html}\label{sec:ranger}
...
\begin{itemize}
	...
	\item \verb\pose [x y z yaw]\: Where the gripper is placed on the robot, relative to the robot's geometric centre. The pose parameter is decribed properly in section \ref{sec:examplerobotsensors}.
\end{itemize}
...
Now in the same way as we built the body we can add on some teeth for Bigbob to collect rubbish between. Figure \ref{fig:bigbobteeth} shows Bigbob with teeth plotted onto a cartesian grid:
\begin{figure}
	\centering
...
% TODO got here 14/04/10

The last sensor that needs adding to Bigbob is the laser, which will be used to detect whenever a piece of rubbish has been collected, the laser's location on the robot is shown in figure \ref{fig:laserdrawing}. Following the same principles as for our previous sensor models we can create a description of this laser:
\begin{verbatim}
...
% TODO find some way of dynamically labelling the appendices
...
When we run the new empty.world with \plst we see our Bigbob robot is occupying the world, as shown in figure \ref{fig:finalRobotBuild}. 
\begin{figure}
	\centering
...
\end{verbatim}
In this bit of code we describe a model called \verb|orange| which uses a bitmap to define its shape and represents an object which is $15cm$ x $15cm$ x $15cm$ and is coloured orange. Figure \ref{fig:orangeandbob} shows our orange model next to Bigbob.
...
\section{Putting the Configuration File Together}\label{sec:finishConfigs}
...
\section{Connecting to the Server and Proxies With Your Code}\label{sec:connectingtoserver}
...
\subsection{Setting Up Connections: an Example.}\label{sec:exampleconnectingtoserver}
...
\section{Using Proxies: A Case Study}
...
\subsection{The Control Architecture}
...
\section*{Appendix A. General Stage Model of Bigbob}\label{app:Abigbob.inc}
\verbatiminput{./appendices/bigbob.inc}

\section*{Appendix B. Worldfile, Containing Robot and Items in World}\label{app:Brobotsjunkworld}
\verbatiminput{./appendices/robots_and_junk.world}

\section*{Appendix C. Configuration file for Bigbob World}\label{app:Cconfig}
\verbatiminput{./appendices/test1.cfg}

\section*{Appendix D. Controlling Code for Bigbob Robot Simulation}\label{app:Dbigbobcode}
