	...
	*/
	...
	//radius of the search circle
      ...
      for(i=0; i<listLength; i++)
      {
            ...
            printf("\t item %d is distance %f\n", i, dist);
            //if the object is not within search circle
            if(dist > (radius + distBotToCircle))
            {
            	//then ignore it
            	continue;
            }
      ...
      if(smallestDist = 1000000)
      {
      	printf("no objects were close enough, false alarm!\n");
      	return -1;
      }
      
      printf("BUGCHECKING. Closest object is %s\n", itemList[closestItem].name);
      double distanceFromBotX = itemList[closestItem].x - robotX;
      double distanceFromBotY = itemList[closestItem].y - robotY;
      distanceFromBotY = (distanceFromBotY*distanceFromBotY) + (distanceFromBotX * distanceFromBotX);
      distanceFromBotY = sqrt(distanceFromBotY);
      
      printf("\t%s is %f metres away.\n", itemList[closestItem].name, distanceFromBotY);
...
{	
      ...
      {		
      	...
      	printf("BUGCHECKING: laser returned %f\n", laserProxy[90]);
