...
\tiobox{First, you'll need to extract the sample code.  To do this, open a
terminal and cd to the directory where you put the file {\tt
tutorial\_code.zip}, then extract using zip\footnote{Yes, there are
GUI-based ways to do this too.  I won't cover them here.}: \\
\\
{\tt > cd \$HOME}\footnote{I'll assume that you put this directory
in your home directory.  If not, just replace the commands given with the
appropriate directory.} \\
{\tt > unzip \$HOME/Downloads/Player-Stage-Manual-master.zip}\footnote{Again, your specific path may differ.} \\
{\tt > cd \$HOME/Player-Stage-Manual-master/code}\footnote{From here on out, I'll
just say {\tt <source\_code> } for brevity and generality}.\\
{\tt > ls} \\
\\
At this point, you should see five directories, {\tt Ch3 Ch4 Ch5.1 Ch5.2
Ch5.3} which contain the code examples for the respective chapters, and
one, {\tt bitmaps} that has pictures used in several different examples.}
...
comes with \plst: \\
{\tt > cd /usr/local/share/stage/worlds} \\
{\tt > player simple.cfg }

Assuming \plst is installed properly you should now have a window open
...
By default, \plst assumes the robot's centre of rotation is at its
geometric centre based on what values are given to the robot's \verb|size|
parameter. Bigbob's \verb|size| is \verb|1.25 x 1 x 1| so \plst will place
...
\subsection{GripperProxy}\label{gripperproxy}
