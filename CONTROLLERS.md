...
to an interface. See Section \ref{sec_Basics_InterfaceDriverDevices}.  On
...
Figure \ref{fig_Coding_ServerClientRobot} shows a basic block diagram of
...
hostname and port is like that discussed in Section
\ref{sec_ConfigurationFile_DeviceAddress}. If your code is running on the
...
port 6665 and the second one 6666 (like in the example of Section
\ref{sec_ConfigurationFile_FinishingCFG}) then you would need two
...
need. The `playerc_blobfinder_blob_t` structure contains the following fields:
	...
	* `x`: The horizontal coordinate of the geometric centre of the blob's bounding box (see Figure \ref{fig_Coding_InteractingWithProxies_Blobfinder_BlobImage}).
	* `y`: The vertical coordinate of the geometric centre of the blob's bounding box (see Figure \ref{fig_Coding_InteractingWithProxies_Blobfinder_BlobImage}).
	* `left`: The horizontal coordinate of the left hand side of the blob's bounding box (see Figure \ref{fig_Coding_InteractingWithProxies_Blobfinder_BlobImage}).
	* `right`: The horizontal coordinate of the right hand side of the blob's bounding box (see Figure \ref{fig_Coding_InteractingWithProxies_Blobfinder_BlobImage}).
	* `top`: The vertical coordinate of the top side of the blob's bounding box (see Figure \ref{fig_Coding_InteractingWithProxies_Blobfinder_BlobImage}).
	* `bottom`: The vertical coordinate of the bottom side of the blob's bounding box (see Figure \ref{fig_Coding_InteractingWithProxies_Blobfinder_BlobImage}).
...
`SetProperty(char *item_name, char *property, void *value, size_t value_len)`
        ...
        * `*_name`: this is the name that you gave to the object in the
          ...
          worldfile. For example, in Section
          \ref{sec_BuildingAWorld_BuildingRobot_ExampleRobot} in the
          worldfile we declared a Bigbob type robot which we called
          ``bob1'' so the `*_name` for that object is ``bob1''. Similarly
          in Section \ref{sec_BuildingAWorld_OtherStuff} we built some
